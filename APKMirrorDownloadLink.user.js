// ==UserScript==
// @name       APKMirror Download Link
// @namespace  *
// @version    1.4.0
// @description  Pushbullet APK Mirror download link upon page load
// @include http*://*apkmirror.com/apk/*
// @copyright  2015, Jordan Hotmann
// @updateURL https://raw.githubusercontent.com/jhotmann/MyUserScripts/master/APKMirrorDownloadLink.user.js
// @downloadURL https://raw.githubusercontent.com/jhotmann/MyUserScripts/master/APKMirrorDownloadLink.user.js
// @icon http://www.apkmirror.com/wp-content/uploads/2015/04/5536e8d182617.png
// @grant        none
// ==/UserScript==

// Pushbullet Access Token
var pbAccessToken = 'PushbulletAccessTokenHere';

// Pushbullet Recipient (only set one of these)
var pbDeviceId = 'SomeDeviceId';
var pbFriendEmail = null;
var pbChannelTag = null;

// Modified code from here https://github.com/alexschneider/pushbullet-js to allow pushing to channels
var PushBullet = (function() {
  ...
  pb.push = function(pushType, devId, email, channel, data, callback) {
    var parameters = {
    ...
    if (channel)
      recipientCount += 1;
    if (recipientCount != 1) {
      var err = new Error('One, and only one, of the following must be set: pbDeviceId, pbFriendEmail, or pbChannelTag');
      if (callback) {
        return callback(err);
      } else {
        throw err;
      ...
      parameters.channel_tag = channel;
    }
...
// The good stuff
PushBullet.APIKey = pbAccessToken;

var downloadsearches = document.getElementsByClassName('downloadButton');
var apkTitles = document.getElementsByClassName('apk-title');
var apkTitle;
if (apkTitles.length > 0)
  apkTitle = apkTitles[0].title;
var notesSections = document.getElementsByClassName('notes-area');
var whatsNew;
if (notesSections.length > 0)
  whatsNew = notesSections[0].innerText;
if (downloadsearches.length > 0) {
  var w = window.prompt('Push ' + apkTitle + ' download link to ' + (pbDeviceId ? pbDeviceId : '') + (pbFriendEmail ? pbFriendEmail : '') + (pbChannelTag ? pbChannelTag : '') + '?', downloadsearches[0].href);
  if (w)
    PushBullet.push('link', pbDeviceId, pbFriendEmail, pbChannelTag, {
      title: apkTitle,
      url: w,
      body: whatsNew
    }, function(err, res) {
      if (err) {
        ...
        console.log('Link pushed successfully!');
      }
    });
}
